...
- `markdown_to_blocks` nests list items (bulleted/numbered/to-do) by comparing each line's leading-whitespace indent (tabs expanded to 4 spaces) against a stack of currently-open list items — any positive indent increase nests as a `children` array under the parent's type key, matching Notion's block-nesting shape. A non-list block (heading, paragraph, quote, etc.) or a flushed paragraph resets the stack, so indentation only nests within a contiguous run of list items.
